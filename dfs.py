...
def calculate_memory_usage(explored, policy=None):
    ...
    Handles both BFS/DFS (just explored states) and MDP algorithms (with value functions and policies).
    """
...
def visualize_search(maze, path, explored, value_function=None):
    ...
    # Title label
    title_label = tk.Label(info_panel, text="DFS Maze Solver", font=("Arial", 16, "bold"), bg="white")
    title_label.pack()
    ...
    def animate_path():
        ...
        if path_idx < len(path):
            ...
            save_metrics_to_csv(["DFS", "Graph Search", len(path), len(unique_explored), memory_usage / 1024, total_time_taken])
